...
plotGoannotation <- function(GOObj,directorypath = NULL)
{
  ...
  # Make the plot
  p = ggplot(data, aes(x=as.factor(data$id), y=value, fill=group)) +       # Note that id is a factor. If x is numeric, there is some space between the first bar


    geom_bar(aes(x=as.factor(data$id), y=value, fill=group), stat="identity", alpha=0.5) +
    ...
    coord_polar() + 
    geom_text(data=label_data, aes(x=label_data$id, y=value+10, label=individual, hjust=label_data$hjust), color="black", fontface="bold",alpha=0.8, size=2.5, angle= label_data$angle, inherit.aes = FALSE ) +
